  ...
  class Importer 
    ...
    def self.import(feed_file, options = {})
      ...
      GtfsReader.config do
        ...
        feed_definition self.feed_definition
        sources do
          gtfs do
            url feed_file
            ...
            handlers do
              agency {|row| 
                  require 'gtfs_api'
                  a = GtfsApi::Agency.new_from_csv(row)
                  a.save! if a.valid?
              ...
              #trips {|row| puts "Read trip: #{row[:trip_id]} #{row[:trip_short_name]}" }
    ...
    def feed_definition
      GtfsReader::FeedDefinition.new.tap do |feed|
